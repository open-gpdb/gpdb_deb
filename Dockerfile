...
RUN cd cbdb && ln -snf /usr/share/zoneinfo/Europe/London /etc/localtime && echo Europe/London > /etc/timezone \
...
&& apt-get update -o Acquire::AllowInsecureRepositories=true && apt-get install -y --no-install-recommends --allow-unauthenticated \
  ...
  ninja-build python-setuptools python3-setuptools quilt unzip wget zlib1g-dev libuv1-dev \
  libgpgme-dev libgpgme11 python2.7 python2.7-dev pkg-config python3.10 python3.10-dev \
  python3-distutils \
&& apt-get clean && rm -rf /var/lib/apt/lists/* \
&& mv cbdb-build/debian ./ 

RUN ls -al .

RUN ln -s /usr/bin/python2.7 /usr/bin/python

ENV DEB_CFLAGS_APPEND='-fno-omit-frame-pointer -fno-lto -fno-fat-lto-objects -Wall -Wno-error=date-time -Wno-error'
ENV DEB_CPPFLAGS_APPEND='-fno-omit-frame-pointer -fno-lto -fno-fat-lto-objects -Wall -Wno-error=date-time -Wno-error'
ENV DEB_CXXFLAGS_APPEND='-fno-omit-frame-pointer -fno-lto -fno-fat-lto-objects -Wall -Wno-error=date-time -Wno-error'
